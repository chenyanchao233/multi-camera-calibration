...
KEYPOINT_CATEGORIES_BY_POSE_MODEL = {
    'COCO-17': ['head', 'head', 'head', 'head', 'head', 'shoulder', 'shoulder', 'elbow', 'elbow', 'hand', 'hand', 'hip',
                'hip', 'knee', 'knee', 'foot', 'foot'],
    'COCO-18': ['head', 'neck', 'shoulder', 'elbow', 'hand', 'shoulder', 'elbow', 'hand', 'hip', 'knee', 'foot', 'hip',
                'knee', 'foot', 'head', 'head', 'head', 'head'],
    'MPII-15': ['head', 'neck', 'shoulder', 'elbow', 'hand', 'shoulder', 'elbow', 'hand', 'hip', 'knee', 'foot', 'hip',
                'knee', 'foot', 'thorax'],
    'MPII-16': ['foot', 'knee', 'hip', 'hip', 'knee', 'foot', 'hip', 'thorax', 'neck', 'head', 'hand', 'elbow',
                'shoulder', 'shoulder', 'elbow', 'hand'],
    'BODY_25': ['head', 'neck', 'shoulder', 'elbow', 'hand', 'shoulder', 'elbow', 'hand', 'hip', 'hip', 'knee', 'foot',
                'hip', 'knee', 'foot', 'head', 'head', 'head', 'head', 'foot', 'foot', 'foot', 'foot', 'foot', 'foot'],
}
...
def generate_pose_pairs_timestamp(poses_2d_timestamp):
    ...
    pose_pairs_2d_timestamp.columns = ['{}_{}'.format(column_name[1], column_name[0]) for column_name in
                                       pose_pairs_2d_timestamp.columns.values]
...
def calculate_3d_poses_camera_pair(
    ...
    if keypoint_a_lengths[0] != keypoint_b_lengths[0]:
        raise ValueError(
            'Keypoint arrays in column A have different number of keypoints than keypoint arrays in column B')
    ...
    pose_pairs_2d_camera_pair['keypoint_coordinates_2d_a_reprojected'] = np.split(keypoints_a_reprojected,
                                                                                  num_pose_pairs)
    pose_pairs_2d_camera_pair['keypoint_coordinates_2d_b_reprojected'] = np.split(keypoints_b_reprojected,
                                                                                  num_pose_pairs)
    if not inplace:
...
def generate_3d_poses_timestamp(
    ...
    global track_labels, diagnostics
    ...
    logger.debug('Finished graph analysis. Returned subgraphs of sizes: {}'.format(
        [subgraph.number_of_nodes() for subgraph in subgraph_list]))
    pose_3d_ids = list()
...
def analyze_pose_subgraph(
    ...
    if best_dispersion_reducing_subgraph is not None:
        ...
        if return_diagnostics:
            subgraph_analysis_diagnostics_item[
                'action'] = 'Dispersion is {}, but removing node {} results in dispersion {}. Done'.format(
                dispersion,
                best_dispersion_reducing_node,
                best_dispersion
            )
    ...
    if best_splitting_subgraph is not None:
        logger.debug(
            'Removing node {} of pose_quality {} results in {} components with k={}. Analyzing each component'.format(
                best_splitting_node,
        ...
        if return_diagnostics:
            subgraph_analysis_diagnostics_item[
                'action'] = 'Dispersion is {}, but removing node {} of pose quality {} results in {} {}-edge-connected components of sizes {}. Analyzing each component'.format(
                dispersion,
                best_splitting_node,
                best_splitting_pose_quality,
                best_splitting_num_components,
    ...
    while True:
        ...
        if return_diagnostics:
            subgraph_analysis_diagnostics_item[
                'action'] = 'Could not find single node that sufficiently reduced dispersion or splits subgraph. ' \
                            'Increasing k to {} splits subgraph into {} components with sizes {}'.format(
                k,
                len(components),
